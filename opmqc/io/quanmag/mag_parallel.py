...
# Notice: Small data is slower than a single thread ( about 4 times slower)
...
def _mag2data(mag_path, check_rf_increment=True, check_ch_zero=True, interpolate_ch_nearest=True,n_jobs=-1,verbose=False):
    """
    ...
    try:
        with open(mag_path, 'rb') as fid:
            ...
            for sec_id in tqdm(range(1, num_sections + 1, 1), file=sys.stdout):
                ...
                # time-consuming step
                cur_pos = fid.tell()  # fileobject index position

                @delayed
                def _read_single_patch(seri_fid, begin_position):
                    ...
                    rf_cnt = list(range(sync_rf_count, sync_rf_count + 20, 2))  # rf pulse cnt incremented by 2;
                    if len(ch_data) != len(rf_cnt):
                        warnings.warn("The size of channel data and the pulse signal are inconsistent!")
                    return (ch_data,rf_cnt)

                serialized_obj = dill.dumps(fid)
                func_params = [cur_pos+56*i for i in range(num_ch_line)]

                single_ch_data_list = Parallel(n_jobs=n_jobs, backend="loky")(_read_single_patch(serialized_obj, args) for args in func_params)

                # handle empty list.
                single_ch_data = [i[0] if i else [] for i in single_ch_data_list]
                single_rf_cnt = [i[1] if i else [] for i in single_ch_data_list]

                single_ch_data = np.array(single_ch_data).flatten()
                single_rf_cnt = np.array(single_rf_cnt).flatten()

                # handle func_params empty.
                if func_params:
                    jump_index = func_params[-1]+56
                else:
                    jump_index = cur_pos
                fid.seek(jump_index, 0)

                single_rf_cnt = np.ceil(single_rf_cnt / 2)
                # check increment(+1)
                ...
                if check_ch_zero:
                    zero_indices = np.where(single_ch_data == 0)[0]
                    if len(zero_indices) > 0:
                        ...
                        if interpolate_ch_nearest:
                            ...
                            single_ch_data = fill_zeros_with_nearest_value(single_ch_data)
            ...
            clogger.info("STEP 2. Parse Trigger Data Blocks")
            ## block header: the number of section. | 8 bytes
            ...
            for tri_sec_id in range(1, trigger_num_sections + 1, 1):
                ...
                clogger.info("trigger_values unique:",np.unique(np.array(trigger_values)))
...
def opmag2fif(mag_path, fif_path, opm_position_path=None, ica_compatibility=True, check_rf_increment=True,
    ...
    if opm_position_path == None:
        opm_position_path = Path(__file__).parent / "data" / "position64.txt"
    ...
    ch_types = ['eeg'] * len(opm_64_ch_names)  # set eeg: only for set_montage() and ICA components compatibility.
...
if __name__=="__main__":
    ...
    raw = opmag2fif(mag_path=opm_mag_dir, fif_path='/Users/reallo/Downloads/opm_artifacts/ta_raw.fif',ica_compatibility=False,verbose=False,check_ch_zero=False,check_rf_increment=False)
    endt = time.time()
    print(f"Time costs:{endt-startt}")
