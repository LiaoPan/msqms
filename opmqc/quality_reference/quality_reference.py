...
from opmqc.io import read_raw_dataset, read_raw_bids_dataset
from opmqc.constants import METRICS_DOMAIN, DATA_TYPE
...
def process_meg_quality(dataset_paths, data_type, file_suffix='.fif', n_jobs=-1,
    ...
    update_reference : bool, optional
        If True, update the reference YAML file in the opmqc library. Default is False.
...
def update_quality_reference_file(quality_reference_df, device_name, overwrite=False):
    """
    Updates the existing quality reference file for a given device name in the opmqc library.

    Automatically determines the opmqc installation directory.
    ...
    try:
        ...
        # Define the path for the YAML file inside opmqc library directory
...
def list_existing_quality_references():
    """
    List all existing quality reference YAML files in the installed opmqc library.

    This function searches the 'quality_reference' directory within the opmqc installation
    ...
    ------
    ImportError
        If the opmqc library is not installed or cannot be found.
    """

    # Define the path to the 'quality_reference' directory within opmqc
    quality_reference_dir = Path(__file__).parent
...
if __name__ == '__main__':
    ...
    # 函数：将新得到的质控区间，更新到opmqc库内部安装目录下yaml文件中，指定质控参考yaml名称；
