...
from .ransac import find_bad_by_ransac
from .removeTrend import removeTrend
from .utils import _filter_design, _mad, _mat_iqr, _mat_quantile


class NoisyChannels:
    """Detect bad channels in an MEG recording using a range of methods.

    This class provides a number of methods for detecting bad channels across a
    full-session MEG recording. Specifically, this class implements all of the
    ...
    At present, only MEG channels are supported and any non-MEG channels in the
    provided data will be ignored.

    Parameters
    ----------
    raw : mne.io.Raw
        An MNE Raw object to check for bad MEG channels.
    do_detrend : bool, optional
        Whether or not low-frequency (<1.0 Hz) trends should be removed from the
        MEG signal prior to bad channel detection. This should always be set to
    ...
    def __init__(self, raw, do_detrend=True, random_state=None, matlab_strict=False):
        ...
        # self.raw_mne.pick_types(meg=True) # Deprecated
        self.raw_mne.pick('meg')
        ...
        # Get original MEG channel names, channel count & samples
        ...
        # Make a subset of the data containing only usable MEG channels
        self.usable_idx = np.isin(ch_names, bads_by_nan_flat, invert=True)
        self.MEGData = self.raw_mne.get_data(picks=ch_names[self.usable_idx])
        self.MEGFiltered = None

        # Get usable MEG channel names & channel counts
        self.ch_names_new = np.asarray(ch_names[self.usable_idx])
        self.n_chans_new = len(self.ch_names_new)

    def _get_filtered_data(self):
        """Apply a [1 Hz - 50 Hz] bandpass filter to the MEG signal.
        ...
        MEG_filt = np.zeros_like(self.MEGData)
        for i in range(MEG_filt.shape[0]):
            MEG_filt[i, :] = signal.filtfilt(bandpass_filter, 1, self.MEGData[i, :])
    ...
    def find_bad_by_nan_flat(self):
        ...
        """
        # Get all MEG channels from original copy of data
        MEGData = self.raw_mne.get_data()

        # Detect channels containing any NaN values
        nan_channel_mask = np.isnan(np.sum(MEGData, axis=1))
        ...
        flat_by_mad = _mad(MEGData, axis=1) < FLAT_THRESHOLD
        flat_by_stdev = np.std(MEGData, axis=1) < FLAT_THRESHOLD
    ...
    def find_bad_by_deviation(self, deviation_threshold=5.0):
        ...
        chan_amplitudes = _mat_iqr(self.MEGData, axis=1) * IQR_TO_SD
    ...
    def find_bad_by_hfnoise(self, HF_zscore_threshold=5.0):
        ...
        if self.MEGFiltered is None:
            self.MEGFiltered = self._get_filtered_data()
        ...
        if self.sample_rate > 100:
            noisiness = np.divide(
                _mad(self.MEGData - self.MEGFiltered, axis=1),
                _mad(self.MEGFiltered, axis=1),
            )
    ...
    def find_bad_by_correlation(
        ...
        for w in range(win_count):
            ...
            meg_filtered = self.MEGFiltered[:, start:end]
            meg_raw = self.MEGData[:, start:end]

            # Get channel amplitude info for the window
            usable = self.usable_idx.copy()
            channel_amplitudes[w, usable] = _mat_iqr(meg_raw, axis=1) * IQR_TO_SD

            # Check for any channel dropouts (flat signal) within the window
            meg_amplitude = _mad(meg_filtered, axis=1)
            dropout[w, usable] = meg_amplitude == 0

            # Exclude any dropout chans from further calculations (avoids div-by-zero)
            usable[usable] = meg_amplitude > 0
            meg_raw = meg_raw[meg_amplitude > 0, :]
            meg_filtered = meg_filtered[meg_amplitude > 0, :]
            meg_amplitude = meg_amplitude[meg_amplitude > 0]

            # Get high-frequency noise ratios for the window
            high_freq_amplitude = _mad(meg_raw - meg_filtered, axis=1)
            noiselevels[w, usable] = high_freq_amplitude / meg_amplitude

            # Get inter-channel correlations for the window
            win_correlations = np.corrcoef(meg_filtered)
        ...
        self._extra_info["bad_by_correlation"] = dict(max_correlations=np.transpose(max_correlations),
                                                      median_max_correlations=np.median(max_correlations, axis=0),
                                                      bad_window_fractions=fraction_bad_corr_windows)
        self._extra_info["bad_by_dropout"] = dict(dropouts=np.transpose(dropout.astype(np.int8)),
